...
const inputDelivery = $('#input-delivery');
const inputContent = $('#input-content');
const orderForm = $('.order-form');
const inputSelect = $('#select-location');
...
const orderSee =$('.order-see-again-food .infor-foods');
...
var youPay = () => {
    return totalPay() + Number(deletePoint(transportFee.innerText.slice(0,-3)));
}

//Hàm xóa dấu chấm
var deletePoint = (myMoney) => {
    let result = myMoney.split('');
    let newArr = result.filter((self) => {
        if(self !== '.'){
            return self;
        }
    });
...
var getTransportFee = (value) => {
    if (value === 'Thành phố Long Xuyên(10.000 VNĐ)') {
        transportFee.innerHTML =`10.000<sup>VND</sup>`;
    }
    else if (value === 'Thị trấn Phú Hòa(9.000 VNĐ)') {
        transportFee.innerHTML =`9.000<sup>VND</sup>`;
    }
    else if (value === 'Thị xã Phước Long(40.000 VNĐ)') {
        transportFee.innerHTML =`40.000<sup>VND</sup>`;
    }
    else if (value === 'Nơi khác(50.000 VNĐ)') {
        transportFee.innerHTML =`50.000<sup>VND</sup>`;
    }
}
...
inputSelect.addEventListener('click', () => {
    ...
    youPayElement.innerHTML = `${formatCash(String(youPay()))}<sup>VND</sup>`;
    ...
    nameFoodsInLocal.forEach((nameFood) => {
        ...
        orderSee.innerHTML += `
            <tr>
                ...
                <td class="money-food"> ${formatCash(String(inforFoodInLocal.price))}<sup>VND</sup></td>
                <td class="btn-change">
                ...
                <td>${formatCash(String(pay(inforFoodInLocal.price, inforFoodInLocal.quantity)))}<sup>VND</sup></td>
...
totalPayElement.innerHTML = `${formatCash(String(totalPay()))}<sup>VND</sup>`;
